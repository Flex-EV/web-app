...
import { fetchRiders } from '@/modules/rider-management/riderManagementSlice.ts';
import { RiderDetails } from '@/modules/rider-management/model/Riders.interface.ts';
...
import { useCustomNavigate } from '@/routes/util/routing.tsx';
...
const Riders = () => {
  ...
  const handleRiderRowClick = (rider: RiderDetails) => {
    navigate.toRiderDetails(rider.id);
  };

  return (
    <FlexContainer title="Rider Management" fullHeight padding="large">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
